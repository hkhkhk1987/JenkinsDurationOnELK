...
public class DatadogHttpRequests {
  ...
  public static void sendEvent(DatadogEvent evt) {
    ...
    try {
      DatadogHttpRequests.curl(evt.createPayload(), DatadogBuildListener.EVENT);
    } catch (Exception e) {
  ...
  /**
   * curl a given {@link JSONObject} payload to the pushgateway service.
   * @param payload - A JSONObject containing a specific subset of a builds metadata.
   * @param type - A String containing the URL subpath pertaining to the type of API post required.
   * @return a boolean to signify the success or failure of the HTTP POST request.
   * @throws IOException if HttpURLConnection fails to open connection
   */
  public static Boolean curl(final JSONObject payload, final String type) throws IOException {
    String hostname = DatadogUtilities.nullSafeGetString(payload, "hostname");
    String number = DatadogUtilities.nullSafeGetString(payload, "number");
    String buildurl = DatadogUtilities.nullSafeGetString(payload, "buildurl");
    String job = DatadogUtilities.nullSafeGetString(payload, "job").replace("/", "_").replace("%2F", "_");
    String jobstatus = DatadogUtilities.nullSafeGetString(payload, "alert_type");
    String duration = DatadogUtilities.nullSafeGetString(payload, "duration");
    String Hostname = DatadogUtilities.getDatadogDescriptor().getHostname();

    try {

      File writename = new File("/opt/" + job + ".txt"); 
    ...
    try {
      ProcessBuilder pb = new ProcessBuilder(
              ...
              "--data-binary",
              "@/opt/" + job + ".txt",
              "http://" + Hostname + ":9091/metrics/job/" + job + "/id/" + number + "");
